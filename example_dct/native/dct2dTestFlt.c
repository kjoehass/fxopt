 ...
 */
#include <stdio.h>
void dct2dTestFlt(float (*dctBlock)[8])
{
  static const float c1   = 0.98078528040323044912; //cos(PI/16)
  static const float s1   = 0.19509032201612826784; //sin(PI/16)
  static const float c3   = 0.83146961230254523708; //cos(3*PI/16)
  static const float s3   = 0.55557023301960222473; //sin(3*PI/16)
  static const float r2c6 = 0.54119610014619698441; //sqrt(2)*cos(6*PI/16)
  static const float r2s6 = 1.30656296487637652784; //sqrt(2)*sin(6*PI/16);
  static const float r2   = 1.41421356237309504880; //sqrt(2);
  int row, col;

  for (row = 0; row < 8; row++) {
    float x0 = dctBlock[row][0], x1 = dctBlock[row][1], x2 =
        dctBlock[row][2], x3 = dctBlock[row][3], x4 =
        dctBlock[row][4], x5 = dctBlock[row][5], x6 =
        dctBlock[row][6], x7 = dctBlock[row][7], x8;
  ...
  for (col = 0; col < 8; col++) {
    float x0 = dctBlock[0][col], x1 = dctBlock[1][col],
